...
export class PopUpOrdenProduccionDetalleEditarComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.hora_produccion = new Date(
      this.fecha_produccion.getFullYear(),
      this.fecha_produccion.getMonth(),
      this.fecha_produccion.getDate(),
      8,
      0,
      0
    );
  ...
  loadGrupo() {
    this.loading = true;
    try {
      this.sectorService.getGrupo().subscribe(
        (resp) => {
          if (resp[0]) {
            this.grupos = resp;
  ...
  calcularHoras(): void {
    const totalTime =
      this.hora_produccion.getHours() * 60 + this.hora_produccion.getMinutes();
    console.log(totalTime);
    console.log(this.selectedRow["unidad_hora"]);
    this.hora_produccion_minutos = totalTime;
    this.cantidad =
      this.hora_produccion_minutos *
      (Number(this.selectedRow["unidad_hora"]) / 60);
    const resultado = this.elementos.findIndex(
      (x) => x.id === this.selectedRow.id
    );
    console.log(resultado);
  ...
  agregarProducto() {
    ...
    if (this.cantidad > 0) {
      ...
      this.elementos[this.index]["packs"] =
        this.cantidad / this.elementos[this.index]["unidades"];
